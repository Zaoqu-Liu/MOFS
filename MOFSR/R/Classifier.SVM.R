...
#' @description This function performs classification using Support Vector Machine (SVM) to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#'
#' @export
Classifier.SVM <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("e1071", "glmnet", "dplyr")
  ...
  if (ncol(d) <= 2) {
    stop("Training data for SVM is insufficient.")
  }
  d <- d[, -1]
  ...
  # Train SVM model
  fit <- e1071::svm(Cluster ~ ., data = d, type = "C-classification", kernel = "radial", probability = TRUE)
  ...
  res1 <- stats::predict(fit, newdata = as.data.frame(test_data_for_prediction), probability = TRUE) %>%
    as.data.frame() %>%
  ...
  res2 <- attr(stats::predict(fit, newdata = as.data.frame(test_data_for_prediction), probability = TRUE), "probabilities") %>%
    as.data.frame() %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
