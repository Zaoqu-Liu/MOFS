...
#' @description This function performs classification using PCA and a neural network to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#'
#' @export
Classifier.PCA <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("glmnet", "dplyr", "nnet", "psych")
  ...
  if (ncol(d) <= 2) {
    stop("Training data for PCA is insufficient.")
  }
  d <- d[, -1]
  ...
  pc <- stats::prcomp(d[, -1], center = TRUE, scale. = TRUE)
  trg <- as.data.frame(predict(pc, d[, -1]))
  trg$Cluster <- d$Cluster

  # Predict PCs for the test data
  tst <- predict(pc, t(data.test[rownames(data.test) %in% colnames(d)[-1], , drop = FALSE]))
  if (ncol(tst) == 0) {
    stop("Test data for prediction is empty.")
  }

  # Train neural network model using top PCs
  fit <- nnet::multinom(Cluster ~ PC1 + PC2 + PC3, trg)

  # Predict cluster assignments for test data
  res1 <- stats::predict(fit, newdata = tst, type = "class") %>%
    as.data.frame() %>%
  ...
  res2 <- stats::predict(fit, newdata = tst, type = "probs") %>%
    as.data.frame() %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
