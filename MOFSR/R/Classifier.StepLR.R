...
#' @description This function performs classification using Stepwise Logistic Regression to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 2. Scales the test data for prediction.
#' 3. Selects genes that are common between the test and training datasets.
#' 4. Uses glmnet to identify the important markers for each cluster and trains a multinomial logistic regression model for classification.
...
#'
#' @export
Classifier.StepLR <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("nnet", "glmnet", "dplyr")
  ...
  if (ncol(d) <= 2) {
    stop("Training data for logistic regression is insufficient.")
  }
  d <- d[, -1]
  ...
  res1 <- stats::predict(fit, newdata = as.data.frame(test_data_for_prediction), type = "class") %>%
    as.data.frame() %>%
  ...
  res2 <- stats::predict(fit, newdata = as.data.frame(test_data_for_prediction), type = "probs") %>%
    as.data.frame() %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
