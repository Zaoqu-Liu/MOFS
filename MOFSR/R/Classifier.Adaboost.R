...
#' @description This function performs classification using AdaBoost to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 5. Uses glmnet to identify the important markers for each cluster and trains an AdaBoost model for classification.
...
#'
#' @export
Classifier.Adaboost <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("glmnet", "adabag", "dplyr")
  ...
  # Prepare training data for AdaBoost
  d <- merge(cluster.data[, 1:2], t(data.train[markersfor, , drop = FALSE]), by.x = "Sample", by.y = 0)
  if (ncol(d) <= 2) {
    stop("Training data for AdaBoost is insufficient.")
  }
  d <- d[, -1]
  ...
  fit <- adabag::boosting(Cluster ~ ., data = d, boos = TRUE, mfinal = 100)
  ...
  res <- stats::predict(fit, newdata = as.data.frame(test_data_for_prediction))

  # Extract probability predictions
  res2 <- as.data.frame(res$prob) %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
  colnames(res2)[-1] <- paste0(levels(cluster.data$Cluster), "_prob")
  res2$Predict <- levels(cluster.data$Cluster)[max.col(res2[, -1], ties.method = "first")]
  return(res2)
}
