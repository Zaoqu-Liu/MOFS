...
#' @description This function performs classification using Ridge Regression to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 5. Uses glmnet to identify the important markers for each cluster and trains a Ridge model for classification.
...
#'
#' @export
Classifier.Ridge <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("glmnet", "dplyr")
  ...
  cluster.markers2 <- lapply(levels(cluster.data$Cluster), function(cluster) {
    ...
    cv.fit <- glmnet::cv.glmnet(data.train_subset, as.numeric(cluster.data$Cluster == cluster),
      family = "binomial", alpha = 0, nfolds = 3, type.measure = "auc"
    )
  ...
  # Prepare training data for Ridge Regression
  d <- merge(cluster.data[, 1:2], t(data.train[markersfor, , drop = FALSE]), by.x = "Sample", by.y = 0)
  if (ncol(d) <= 2) {
    stop("Training data for Ridge Regression is insufficient.")
  }
  d <- d[, -1]
  ...
  set.seed(123)
  cv.fit <- glmnet::cv.glmnet(as.matrix(d[, -1]), d[, 1],
    family = "multinomial", alpha = 0, nfolds = 3, type.measure = "mse"
  )
  ...
  res1 <- stats::predict(cv.fit, s = cv.fit$lambda.min, newx = test_data_for_prediction, type = "class") %>%
    as.data.frame() %>%
  ...
  res2 <- stats::predict(cv.fit, s = cv.fit$lambda.min, newx = test_data_for_prediction, type = "response") %>%
    as.data.frame() %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
