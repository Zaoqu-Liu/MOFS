...
#' @description This function performs classification using Naive Bayes to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 5. Uses glmnet to identify the important markers for each cluster and trains a Naive Bayes model for classification.
...
#'
#' @export
Classifier.NBayes <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("glmnet", "e1071", "dplyr")
  ...
  if (ncol(d) <= 2) {
    stop("Training data for Naive Bayes is insufficient.")
  }
  d <- d[, -1]
  ...
  fit <- e1071::naiveBayes(x = as.matrix(d[-1]), y = d$Cluster, laplace = 0)
