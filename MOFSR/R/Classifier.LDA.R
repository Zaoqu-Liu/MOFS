...
#' @description This function performs classification using Linear Discriminant Analysis (LDA) to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 5. Uses glmnet to identify the important markers for each cluster and trains an LDA model for classification.
...
#'
#' @export
Classifier.LDA <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("MASS", "glmnet", "dplyr")
  ...
  res1 <- as.data.frame(res$posterior) %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
  colnames(res1)[-1] <- paste0(levels(cluster.data$Cluster), "_prob")
  res1$Predict <- levels(cluster.data$Cluster)[apply(res1[, -1], 1, which.max)]
