...
#' @description This function performs classification using Gradient Boosted Decision Trees (GBDT) to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 5. Uses glmnet to identify the important markers for each cluster and trains a Gradient Boosted Decision Trees model for classification.
...
#'
#' @export
Classifier.GBDT <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("glmnet", "dplyr", "gbm")
  ...
  if (ncol(d) <= 2) {
    stop("Training data for GBDT is insufficient.")
  }
  d <- d[, -1]
  ...
  # Train GBDT model
  set.seed(123)
  fit <- zquiet(gbm::gbm(Cluster ~ .,
    distribution = "multinomial",
    data = d, n.trees = 10000, shrinkage = 0.01
  ))
  best.iter <- zquiet(gbm::gbm.perf(fit, plot.it = FALSE))
  ...
  res <- stats::predict(fit, newdata = as.data.frame(test_data_for_prediction), n.trees = best.iter, type = "response") %>% as.data.frame()

  # Extract probability predictions
  res <- res %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
  colnames(res)[-1] <- paste0(levels(cluster.data$Cluster), "_prob")
  res$Predict <- levels(cluster.data$Cluster)[apply(res[, -1], 1, which.max)]
