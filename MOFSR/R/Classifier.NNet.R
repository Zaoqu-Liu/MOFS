...
#' @description This function performs classification using a neural network to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 5. Uses glmnet to identify the important markers for each cluster and trains a neural network model for classification.
...
#'
#' @export
Classifier.NNet <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  ...
  if (ncol(d) <= 2) {
    stop("Training data for Neural Network is insufficient.")
  }
  d <- d[, -1]
  colnames(d) <- gsub("-", "_", colnames(d))
  rownames(data.test) <- gsub("-", "_", rownames(data.test))
  nh <- nrow(d) / (2 * (length(markersfor) + 3))
  if (nh <= 3) {
    nh <- 4
  }

  fit <- nnet::nnet(Cluster ~ ., d, size = nh, rang = max(abs(d[, -1])), decay = 5e-4, maxit = 200)
  ...
  res2 <- stats::predict(fit, newdata = test_data_for_prediction, type = "raw") %>%
    as.data.frame() %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
