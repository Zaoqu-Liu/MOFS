...
#' @description This function performs classification using Elastic Net to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 5. Uses glmnet to identify the important markers for each cluster and trains an Elastic Net model for classification.
...
#'
#' @export
Classifier.Enet <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  ...
  cluster.markers2 <- lapply(levels(cluster.data$Cluster), function(cluster) {
    ...
    cv.fit <- glmnet::cv.glmnet(data.train_subset, as.numeric(cluster.data$Cluster == cluster),
      family = "binomial", alpha = 0.5, nfolds = 3, type.measure = "auc"
    )
  ...
  # Prepare training data for Elastic Net
  d <- merge(cluster.data[, 1:2], t(data.train[markersfor, , drop = FALSE]), by.x = "Sample", by.y = 0)
  if (ncol(d) <= 2) {
    stop("Training data for Elastic Net is insufficient.")
  }
  d <- d[, -1]
  ...
  cv.fit <- glmnet::cv.glmnet(as.matrix(d[, -1]), d[, 1],
    family = "multinomial", alpha = 0.5, nfolds = 3, type.measure = "mse"
  )
