...
#' @description This function performs classification using Random Forest to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 3. Scales the test data for prediction if `scale` is TRUE.
#' 4. Selects genes that are common between the test and training datasets.
#' 5. Uses glmnet to identify the important markers for each cluster and trains a Random Forest model for classification.
...
#'
#' @export
Classifier.RF <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("randomForest", "glmnet", "dplyr", "tibble")
  ...
  if (ncol(d) <= 2) {
    stop("Training data for Random Forest is insufficient.")
  }
  d <- d[, -1]
  ...
  set.seed(1234)
  rf <- randomForest::randomForest(Cluster ~ ., data = d, ntree = 1000, nodesize = 3)
  ...
  res1 <- predict(rf, newdata = as.data.frame(test_data_for_prediction), type = "response") %>%
    as.data.frame() %>%
    tibble::rownames_to_column("ID") %>%
    dplyr::rename("Predict" = ".")

  # Predict cluster probabilities for test data
  res2 <- predict(rf, newdata = as.data.frame(test_data_for_prediction), type = "prob") %>%
    as.data.frame() %>%
    tibble::rownames_to_column("ID")
