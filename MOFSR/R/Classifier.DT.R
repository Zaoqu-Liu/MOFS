...
#' @description This function performs classification using Decision Tree to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 5. Uses glmnet to identify the important markers for each cluster and trains a Decision Tree model for classification.
...
#'
#' @export
Classifier.DT <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("glmnet", "rpart", "dplyr")
  ...
  if (ncol(d) <= 2) {
    stop("Training data for Decision Tree is insufficient.")
  }
  d <- d[, -1]
  ...
  tree <- rpart::rpart(Cluster ~ ., data = d, method = "class", parms = list(split = "gini"), control = list(minbucket = 3))
  pruned.tree <- rpart::prune(tree, cp = tree$cptable[which.min(tree$cptable[, "xerror"]), "CP"])
  ...
  res1 <- stats::predict(pruned.tree, newdata = as.data.frame(test_data_for_prediction), type = "class") %>%
    as.data.frame() %>%
  ...
  res2 <- stats::predict(pruned.tree, newdata = as.data.frame(test_data_for_prediction), type = "prob") %>%
    as.data.frame() %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
