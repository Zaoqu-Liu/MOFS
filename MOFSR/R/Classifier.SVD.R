...
#' @description This function performs classification using SVD and a neural network to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 5. Uses glmnet to identify the important markers for each cluster and performs SVD to reduce dimensionality.
#' 6. Trains a neural network model for classification using the top singular vectors.
...
#'
#' @export
Classifier.SVD <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  # Install required packages if not already installed
  required_packages <- c("glmnet", "nnet", "dplyr")
  ...
  if (ncol(d) <= 2) {
    stop("Training data for SVD is insufficient.")
  }
  d <- d[, -1]
  ...
  svd_res <- svd(d[, -1])
  td <- as.data.frame(svd_res$u)
  td$Cluster <- d$Cluster
  vd <- as.data.frame(svd(t(data.test[rownames(data.test) %in% colnames(d)[-1], , drop = FALSE]))$u)
  rownames(vd) <- colnames(data.test)

  # Train neural network model
  fit <- nnet::multinom(Cluster ~ V1 + V2 + V3, data = td)

  # Predict cluster assignments for test data
  test_data_for_prediction <- vd
  res1 <- stats::predict(fit, newdata = test_data_for_prediction, type = "class") %>%
    as.data.frame() %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
  colnames(res1)[2] <- "Predict"

  # Predict cluster probabilities for test data
  res2 <- stats::predict(fit, newdata = test_data_for_prediction, type = "probs") %>%
    as.data.frame() %>%
    dplyr::mutate(ID = colnames(data.test)) %>%
    dplyr::select(ID, everything())
  colnames(res2)[-1] <- paste0(levels(cluster.data$Cluster), "_prob")

  # Merge prediction results
  res3 <- dplyr::left_join(res2, res1, by = "ID")
