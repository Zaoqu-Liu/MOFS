...
#' @description This function performs classification using LASSO (Least Absolute Shrinkage and Selection Operator) to predict cluster assignments for test data based on trained models from training data and cluster markers.
...
#' 5. Uses glmnet to identify the important markers for each cluster and trains a LASSO model for classification.
...
#'
#' @export
Classifier.LASSO <- function(data.test, data.train, cluster.data, cluster.markers, scale = TRUE) {
  ...
  if (ncol(d) <= 2) {
    stop("Training data for LASSO is insufficient.")
  }
  d <- d[, -1]
  ...
  cv.fit <- glmnet::cv.glmnet(as.matrix(d[, -1]), d[, 1],
    family = "multinomial", alpha = 1, nfolds = 3, type.measure = "mse"
  )
