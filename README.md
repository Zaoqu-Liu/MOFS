...
This R script is designed to perform multi-modality clustering, integrating different biological data types such as mutation, Copy Number Variation (CNV), RNA expression, protein expression, pathology, and radiology imaging. The primary objective of this analysis is to explore the optimal number of clusters in a multi-modality dataset by leveraging two different approaches: Nonnegative Matrix Factorization (NMF) and Principal Component Analysis (PCA). These methods are particularly effective for identifying distinct biological subgroups within complex datasets, which can be useful for understanding disease mechanisms and discovering new therapeutic targets.

The script works by combining different feature subset sizes of each modality type, iteratively assessing the clustering performance for each combination. The clustering quality is quantified using two metrics: Cluster Prediction Index (CPI) and the GAP statistic. The CPI is calculated using the IntNMF R package (PMID: 28459819), which evaluates the consistency of clustering results across multiple iterations by assessing how well the features and samples are grouped together in the final clustering solution. Essentially, CPI quantifies the stability of the cluster assignments when subsets of the data are used for clustering. The higher the CPI, the more stable and robust the clustering solution is, indicating that the chosen features contribute meaningfully to the biological classification.

The GAP statistic, on the other hand, is calculated using the mogsa R package (PMID: 31243065). This metric is useful for determining the optimal number of clusters by comparing the intra-cluster dispersion of the actual dataset with that of a randomly generated reference dataset. Specifically, the GAP statistic compares the compactness of clusters in the real data to those in a dataset with uniformly distributed random values, helping to identify whether the observed clustering structure is likely to reflect meaningful biological groupings or just random noise. The optimal number of clusters is identified by the value of k (the number of clusters) where the GAP statistic reaches its maximum, indicating a significant separation between clusters.

To ensure the reproducibility of our analysis, the detailed code for the computations, including both CPI and GAP calculations, is made available at our GitHub repository: GitHub Repository. This repository provides all necessary scripts and data preprocessing steps required to reproduce the clustering analysis described in our research.

### Median Absolute Deviation

In this script, feature subsets are selected based on their variance using the Median Absolute Deviation (MAD) method. For each feature type (RNA, protein, pathology, and radiology), the features are ranked by MAD, which measures the variability of each feature across samples. Higher MAD values indicate more variability, which is useful for identifying features that are likely to contribute to cluster differentiation. Feature subset sizes are varied systematically, and multiple combinations are evaluated in order to determine which combination of feature sizes leads to the most robust clustering solution. This exhaustive search strategy helps to ensure that the selected features are biologically relevant and contribute to the stability and reproducibility of the identified clusters.

### Parallel Computing

The script also employs parallel computing to speed up the analysis, given the large number of combinations of feature subset sizes and cluster numbers that are tested. The future.apply package is used to distribute the computations across multiple cores, making the process more efficient. For each combination of feature subset sizes, the clustering analysis is run multiple times using NMF and mbPCA. The CPI is calculated by running the NMF multiple times for each cluster number (k) and measuring the stability of the clustering solution. The GAP statistic is calculated using mbPCA, which integrates multiple modality datasets and extracts components that represent shared variation across the datasets. Clustering is performed on these integrated components, and the robustness of the resulting clusters is quantified using the GAP statistic.

### Output

The final output of the script is a dataframe that contains the results for each combination of feature subset sizes and cluster numbers. For each combination, both the CPI and GAP statistic are calculated, and a combined score is computed as the sum of CPI and GAP. This combined score provides a comprehensive measure of clustering quality, taking into account both the stability of the clusters (CPI) and the separation between clusters (GAP). The combination with the highest score is considered to be the best clustering solution, as it indicates both high stability and well-separated clusters. This analysis provides valuable insights into the biological structure of the data and can help identify meaningful subtypes in complex multi-modality datasets.
